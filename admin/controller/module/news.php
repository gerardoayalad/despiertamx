...
class ControllerModuleNews extends Controller {
	...
	public function insertCategory() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateFormCategory())) {
			$this->model_fido_news->addNewsCategory($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('module/news/listingcategory', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function updateCategory() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateFormCategory())) {
			$this->model_fido_news->editNewsCategory($this->request->get['category_id'], $this->request->post);
	...
	public function deleteCategory() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $category_id) {
				$this->model_fido_news->deleteNewsCategory($category_id);
			}
	...
	private function getListCategory() {
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('module/news/listingcategory', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title_category'),
			'separator' => ' :: '
		);
		
		$this->data['newslist']   = $this->url->link('module/news/listing', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['module'] = $this->url->link('module/news', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['category'] = $this->url->link('module/news/category', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['insert'] = $this->url->link('module/news/insertCategory', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('module/news/deleteCategory', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['news'] = array();

		$news_total = $this->model_fido_news->getTotalNewsCategory();
		
		$results = $this->model_fido_news->getNewsCat();

		foreach ($results as $result) {
			$action = array();
			$action[] = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('module/news/updateCategory', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'], 'SSL')
			);
		...
		$this->template = 'module/news/list-category.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getCategoryForm() {
		...
		if (!isset($this->request->get['category_id'])) {
			$this->data['action'] = $this->url->link('module/news/insertCategory', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('module/news/updateCategory', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'], 'SSL');
		}

		$this->data['cancel'] = $this->url->link('module/news/listingcategory', 'token=' . $this->session->data['token'], 'SSL');
		
		if ((isset($this->request->get['category_id'])) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$news_info = $this->model_fido_news->getNewsCat($this->request->get['category_id']);
		}
		...
		if (isset($this->request->post['news_description'])) {
			...
			$this->data['news_description'] = $this->model_fido_news->getNewsCategory($this->request->get['category_id']);
		} else {
			$this->data['news_description'] = array();
		}

		$this->template = 'module/news/form-category.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm() {
		...
		$this->data['news_category_data'] = $this->model_fido_news->getNewsAllCategory();
