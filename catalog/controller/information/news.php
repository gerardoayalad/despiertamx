...
class ControllerInformationNews extends Controller {
	public function index() {
		...
		$category_data =  $this->model_fido_news->getCategories();
		foreach ($category_data as $result) {
			$this->data['category_data'][] = array(
				'title'        => $result['title'],
				'href'         => $this->url->link('information/news', 'category_id=' . $result['category_id']),
			);
		}
		...
		if ($news_info) {
			...
			if(isset($category_id)){
				$news_data = $this->model_fido_news->getNewsCategory($category_id);
			}else{
