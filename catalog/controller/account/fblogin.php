...
class ControllerAccountFblogin extends Controller {
	private $error = array();
  	...
  	public function index() {
		...
		if ($fbuser){
			try {
				$fbuser_profile = $this->fblogin->api("/me");
		...
		if($fbuser_profile['id'] && $fbuser_profile['email']){
			...
			if($email_query->num_rows){
				...
				$add_data['fax'] = '';
                                $add_data['facebook_id'] = (int)$fbuser_profile['id'];
				$add_data['telephone'] = '';
				$add_data['company'] = '';
				$add_data['address_1'] = '';
				$add_data['address_2'] = '';
				...
				$add_data['zone_id'] = 0;
                                ...
                                $this->model_account_customer->addFBToCustomer($add_data['facebook_id'],$add_data['email']);
