...
class ModelFidoNews extends Model {
	...
	public function getCategories() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_category n LEFT JOIN " . DB_PREFIX . "news_category_language nd ON (n.category_id = nd.category_id)  WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "'  AND n.status = '1' ORDER BY n.date_added DESC");
		return $query->rows;
	}
	
	public function getNewsCategory($category_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) LEFT JOIN " . DB_PREFIX . "news_to_store n2s ON (n.news_id = n2s.news_id) WHERE nd.category_id = '" . (int)$category_id . "' AND nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND n.status = '1'");
		return $query->rows;
	}
